    ...
    class RelationsProcessor : IRelationsProcessor
    {
            ...
            foreach (var member in relation.Members)
            {
                ...
                switch (member.Type)
                {
                    ...
                    case "relation":
                        ...
                        break;
                    default:
                        throw new ApplicationException("Didn't expect this");
                }
        ...
        private static void ProcessRelationFullGeometry(Relation newRelation, List<Node> nodes, List<Way> psuedoWays, List<Way> ways)
        {
            ...
            foreach (var member in newRelation.Members)
            {
                ...
                if (member.Type == "node")
                {
                    if (member.Lat.HasValue) nodes.Add(new Node { Id = member.Ref.ToString(CultureInfo.InvariantCulture), Lat = member.Lat.Value, Lon = member.Lon.Value, Parent = newRelation});
                }
